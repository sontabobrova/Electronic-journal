...
1. Создать `.env`:
   ...
   Для демонстрационного стенда с доступом с других машин:
   ...
   В `.env.release` перед запуском нужно заменить домены, `DJANGO_SECRET_KEY` и пароль PostgreSQL.
...
Если сервисы запущены на сервере или виртуальной машине, а интерфейс открывается с другой машины, браузер должен обращаться к backend по адресу этого же сервера.

Для demo-стенда используйте `.env.demo`:
...
В режиме `auto` frontend строит адрес API как `http://<адрес-сервера>:8000`, где `<адрес-сервера>` берется из адресной строки браузера.

После изменения `.env` пересоздайте контейнеры, чтобы env-переменные попали в `backend` и `frontend`:

```bash
docker compose up -d --force-recreate backend frontend
```

Открывать приложение с другой машины нужно по адресу:

```text
http://<адрес-сервера>:3000/
```

Важно: `VITE_API_BASE_URL=http://localhost:8000` подходит только когда браузер открыт на той же машине, где доступен backend. Для сетевого demo-запуска используйте `VITE_API_BASE_URL=auto`. Для релиза используйте домен сервера, например `https://journal.example.com`.
...
- `.env.release` - релизная конфигурация с отключенным debug, доменом сервера и placeholder-секретами.
